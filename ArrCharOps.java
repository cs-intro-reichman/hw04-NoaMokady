...
public class ArrCharOps {
    ...
    public static long hashCode(char[] arr) {
        Integer hashInt = 0;
        for (int i = 0; i < arr.length; i++) {
            hashInt += (int) (arr[i] * Math.pow(7, arr.length - (i + 1)));
        }
        return hashInt;
    }
    ...
    public static int compareTo(String str1, String str2) {
        // If one of the strings is empty, returns -2.
        if (str1.length() == 0 || str2.length() == 0) {
            return -2;
        }

        // Compares the lexicographically size of characters in the same position.
